...
def update_table(n_clicks, year_chosen, comp_chosen, age_chosen):
    ...
    df_chosen.drop(["Competition", "Year", "Age Group"], axis=1, inplace = True)
    ...
    fig_by_judge = go.Figure(
        data=[go.Scatter(x = x, y = yi, name = df_chosen['Name'].iloc[i]) for i,yi in enumerate(y)]
        )

    fig_by_judge.update_yaxes(autorange = "reversed")
    fig_by_judge.update_layout(title_text='Placement Visualized',
                      title_x=0.5,
                      font=dict(size=12),
                      title_font_color="black",
                      title_font_family="Arial",)
    ...
    dcc.Tabs(id="tabs", value='tab_overall', children=[
        dcc.Tab(label='Overall Results', value='tab_overall', children = dcc.Graph(figure=fig_overall)),
        # dcc.Tab(label='Championship Results', value='tab_by_judge', children = dcc.Graph(figure=fig_by_judge)),
    ]),
            
    ]

    children = [
        html.P("Results", style = {'textAlign': 'center', 'fontSize' : '18'}),
        dbc.Row(table_card),
        html.Br(),
        dbc.Row(plot_card)
    ]
